...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    JAVA_OPTS_PATTERN="(^(\"$JAVA_OPTS\")|(-Djava.awt.headless=.*))"
    GRADLE_OPTS_PATTERN="(^(\"$GRADLE_OPTS\")|(-Dorg.gradle.appname=.*))"
    # Now convert the arguments - kludge to limit ourselves to /bin/sh
    i=0
    for arg in "$@" ; do
        CHECK=`echo "$arg"|egrep -c "$JAVA_OPTS_PATTERN" 2>/dev/null`
        CHECK2=`echo "$arg"|egrep -c "$GRADLE_OPTS_PATTERN" 2>/dev/null`
